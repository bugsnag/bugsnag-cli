Feature: React Native 0.69 iOS Integration Tests
  ...
  Scenario: Build and Upload React Native 0.69 iOS sourcemaps
    ...
    When I run bugsnag-cli with upload react-native-ios --upload-api-root-url=http://localhost:9339 --api-key=1234567890ABCDEF1234567890ABCDEF --scheme=rn0_69 --overwrite --verbose features/base-fixtures/rn0_69
    And I wait to receive 1 sourcemaps
    Then the sourcemap is valid for the React Native Build API
    Then the sourcemaps Content-Type header is valid multipart form-data
    And the sourcemap payload field "apiKey" equals "1234567890ABCDEF1234567890ABCDEF"
  ...
  Scenario: Build, Archive and Upload React Native 0.69 iOS sourcemaps
    ...
    When I run bugsnag-cli with upload react-native-ios --upload-api-root-url=http://localhost:9339 --api-key=1234567890ABCDEF1234567890ABCDEF --overwrite --verbose features/base-fixtures/rn0_69
    And I wait to receive 1 sourcemaps
    Then the sourcemap is valid for the React Native Build API
    Then the sourcemaps Content-Type header is valid multipart form-data
    And the sourcemap payload field "apiKey" equals "1234567890ABCDEF1234567890ABCDEF"
