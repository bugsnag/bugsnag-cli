Feature: React Native 0.72 Cocoa Integration Tests
  ...
  Scenario: Build and Upload React Native 0.72 Cocoa sourcemaps
    ...
    When I run bugsnag-cli with upload react-native-cocoa --upload-api-root-url=http://localhost:9339 --api-key=1234567890ABCDEF1234567890ABCDEF --overwrite --dev features/base-fixtures/rn0_72
    And I wait to receive 1 sourcemaps
    Then the sourcemap is valid for the React Native Build API
    Then the sourcemaps Content-Type header is valid multipart form-data
    And the sourcemap payload field "apiKey" equals "1234567890ABCDEF1234567890ABCDEF"
