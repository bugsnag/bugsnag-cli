Feature: dSYM Upload Integration Tests
  ...
  Scenario: Build and upload a dSYM sourcemap using path containing a xcodeproj file
    When I make the "features/base-fixtures/dsym/swift-package-manager"
    And I wait for the build to succeed

    When I run bugsnag-cli with upload dsym --upload-api-root-url=http://localhost --api-key=1234567890ABCDEF1234567890ABCDEF features/base-fixtures/dsym/swift-package-manager
    And I wait to receive 1 sourcemaps
    Then the sourcemap is valid for the dSYM Build API
    Then the sourcemaps Content-Type header is valid multipart form-data
    And the sourcemap payload field "apiKey" equals "1234567890ABCDEF1234567890ABCDEF"
