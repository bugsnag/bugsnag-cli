Feature: dSYM Expected Error and Warning scenario Integration Tests
  ...
  Scenario: If --ignore-empty-dsym is set to true, then the log message returned should be [WARN]
    When I run bugsnag-cli with upload xcode-build --upload-api-root-url=http://localhost:$MAZE_RUNNER_PORT --api-key=1234567890ABCDEF1234567890ABCDEF --project-root=/path/to/project/root --scheme=test --ignore-empty-dsym=true features/xcode/fixtures/ZeroByteDsym
