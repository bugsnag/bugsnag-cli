Feature: Minidump Integration Tests
    ...
    Scenario: Upload a single Minidump sym using all CLI flags
        When I run bugsnag-cli with upload minidump --upload-api-root-url=http://localhost:9339 --version-code=2 --version-name=2.0 --OsName="Linux" --cpu-info="x86_64" --product="test-product" --api-key=1234567890ABCDEF1234567890ABCDEF --overwrite features/minidump/fixtures/breakpad-symbols.sym
