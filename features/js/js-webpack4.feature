Feature: Webpack 4 js Integration Tests
  ...
  Scenario: Automatically resolves the version number based on the package.json
    When I run bugsnag-cli with upload js --upload-api-root-url=http://localhost:9339 --api-key=1234567890ABCDEF1234567890ABCDEF --overwrite --bundle-url=example.com features/js/fixtures/js-webpack4/dist
    And I wait to receive 1 sourcemaps
    Then the sourcemap is valid for the JS Build API
    Then the sourcemaps Content-Type header is valid multipart form-data
    And the sourcemap payload field "apiKey" equals "1234567890ABCDEF1234567890ABCDEF"
  ...
  Scenario: Resolves the path specified as the map
    When I run bugsnag-cli with upload js --upload-api-root-url=http://localhost:9339 --api-key=1234567890ABCDEF1234567890ABCDEF --overwrite --bundle-url=example.com features/js/fixtures/js-webpack4/dist/main.js.map
    And I wait to receive 1 sourcemaps
    Then the sourcemap is valid for the JS Build API
    Then the sourcemaps Content-Type header is valid multipart form-data
    And the sourcemap payload field "apiKey" equals "1234567890ABCDEF1234567890ABCDEF"
  ...
  Scenario: Searches in the dist folder automatically
    When I run bugsnag-cli with upload js --upload-api-root-url=http://localhost:9339 --api-key=1234567890ABCDEF1234567890ABCDEF --overwrite --bundle-url=example.com features/js/fixtures/js-webpack4/
    And I wait to receive 1 sourcemaps
    Then the sourcemap is valid for the JS Build API
    Then the sourcemaps Content-Type header is valid multipart form-data
    And the sourcemap payload field "apiKey" equals "1234567890ABCDEF1234567890ABCDEF"
  ...
  Scenario: Build and Upload js webpack4 sourcemaps
    ...
    When I run bugsnag-cli with upload js --upload-api-root-url=http://localhost:9339 --api-key=1234567890ABCDEF1234567890ABCDEF --overwrite --bundle-url=example.com features/base-fixtures/js-webpack4/dist/main.js.map
    And I wait to receive 1 sourcemaps
    Then the sourcemap is valid for the JS Build API
    Then the sourcemaps Content-Type header is valid multipart form-data
    And the sourcemap payload field "apiKey" equals "1234567890ABCDEF1234567890ABCDEF"
