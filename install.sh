...
have_sudo_access() {
  ...
  if [[ -n "${BUGSNAG_CLI_ON_MACOS-}" ]] && [[ "${HAVE_SUDO_ACCESS}" -ne 0 ]]
  then
...
if [[ -d "${BUGSNAG_CLI_PREFIX}" && ! -x "${BUGSNAG_CLI_PREFIX}" ]]
then
  abort "$(
    cat <<EOABORT
...
do
  if exists_but_not_writable "${BUGSNAG_CLI_PREFIX}/${dir}"
  then
...
if [[ ":${PATH}:" != *":${BUGSNAG_CLI_PREFIX}/bin:"* ]]
then
...
if [[ "$(which bugsnag-cli)" != "${BUGSNAG_CLI_PREFIX}/bin/bugsnag-cli" ]]
then
  cat <<EOS
